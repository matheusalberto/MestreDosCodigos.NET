...
* _Diferentemente do .Net Framework, o .NET Core é multiplataforma, além de possuir alta performance e sistemas escaláveis. O .NET Core tem um intuito do uso de microsserviços e possuir maior modularidade para o uso de containeres._
...
### Para executar os Exercícios, basta clonar o repositório e abri-lo no Visual Studio. Para cada exercício foi criado um projeto separado.
...
Para completar (testar) este exercício foi necessário instalar o Ubuntu. Fiz isso aplicando os seguintes passos:
	- Fazer o download do Ubuntu na Microsoft Store. 
	- Ir em Painel de Controle -> Ativar ou desativar Recursos do Windows e marcar a opção "Subsistema do Windows para Linux".
	- Ao marcar esta opção, será necessário reiniciar o computador (reinicie após download do Ubuntu concluído).
	- Após reiniciar, execute o aplicativo "Ubuntu" e siga as próximas instruções.

Completado os passos anteriores, basta através da linha de comando no windows, navegar até o exercício desejado e digitar: "dotnet publish -c release -r ubuntu.16.04-x64". 

Terminado o processo, basta navegar através do aplicativo Ubuntu até a pasta 'publish' criada após comando anterior e executar: "./NomeDoExercicio", e o exercício vai ser executado dentro de um ambiente linux.
